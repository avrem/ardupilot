...
void QuadPlane::vtol_position_controller(void)
{
    ...
    case QPOS_POSITION2: {
        ...
        if (plane.control_mode == QRTL || plane.control_mode == GUIDED || vtol_loiter_auto) {
            ...
            pos_control->set_alt_target(target_altitude - inertial_nav.get_origin().alt);
        } else {
...
void QuadPlane::Log_Write_QControl_Tuning()
{
    ...
    struct log_QControl_Tuning pkt = {
        ...
        weathervane_rate    : weathervane.last_output,
    };
...
int8_t QuadPlane::forward_throttle_pct(void)
{
    ...
    */
    ...
    // WP nav speed.
    fwd_vel_error -= (wp_nav->get_speed_xy() * 0.01f) * plane.nav_pitch_cd / (float)aparm.angle_max;
...
float QuadPlane::get_weathervane_yaw_rate_cds(void)
{
    ...
    */
    ...
    if (psc_accel_xy.length() > min_accel) {
        ...
        output = constrain_float(yaw_error * weathervane.gain, -yaw_rate_max * 100, yaw_rate_max * 100);
    }
