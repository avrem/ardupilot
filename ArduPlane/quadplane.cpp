...
void QuadPlane::multicopter_attitude_rate_update(float yaw_rate_cds)
{
    ...
    if (in_vtol_mode() || is_tailsitter() || (is_tiltquad() && !use_fw_rates)) {
        ...
        attitude_control->input_rate_bf_roll_pitch_yaw_2(roll_rate*100.0f, pitch_rate*100.0f, yaw_rate_cds, use_fw_rates == 2);
    }
}
    ...
    case TRANSITION_AIRSPEED_WAIT: {
        ...
        if (have_airspeed && aspeed > plane.aparm.airspeed_min && !assisted_flight && plane.relative_ground_altitude(plane.g.rangefinder_landing) >= tran_alt_max) {
            ...
            if (is_tiltquad()) { // go directly to TRANSITION_DONE
                ...
                transition_start_ms = 0;
                transition_low_airspeed_ms = 0;
                return; // prevent inconsistent TRANSITION_DONE/assisted_flight state
            }
        }
        assisted_flight = true;
