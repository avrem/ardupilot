...
#define AC_ATTITUDE_THRUST_ERROR_ANGLE                  radians(300.0f) // Thrust angle error above which yaw corrections are limited
...
protected:
    ...
    virtual float rate_target_to_motor_roll(float rate_actual_rads, float rate_target_rads);

    // Run the pitch angular velocity PID controller and return the output
    virtual float rate_target_to_motor_pitch(float rate_actual_rads, float rate_target_rads);
