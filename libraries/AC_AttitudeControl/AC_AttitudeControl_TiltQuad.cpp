// -*- tab-width: 4; Mode: C++; c-basic-offset: 4; indent-tabs-mode: t -*-

#include "AC_AttitudeControl_TiltQuad.h"
#include <AP_Math/AP_Math.h>
#include "ElytraConfigurator.h"

// rate_controller_run - run lowest level rate controller and send outputs to the motors
// should be called at 100hz or more
void AC_AttitudeControl_TiltQuad::rate_controller_run()
{
    _motors.set_roll(aeroxo_rate_bf_to_motor_roll(0));
    _motors.set_pitch(aeroxo_rate_bf_to_motor_pitch(0));
    _motors.set_yaw(aeroxo_rate_bf_to_motor_yaw(_ang_vel_target_rads.z));
}

float AC_AttitudeControl_TiltQuad::aeroxo_rate_bf_to_motor_roll(float rate_target_rads)
{
    float current_rate_rads = _ahrs.get_gyro().x;
    float rate_error_rads = rate_target_rads - current_rate_rads;  
    float angle_error_rads = _att_error_rot_vec_rad.x;

    float p = _pi_stabilize_roll.kP() * angle_error_rads * _conv / 1000.0f + _pi_stabilize_roll_tilt.kP() * angle_error_rads * (1000 - _conv) / 1000.0f;
    float i = _pi_stabilize_roll.get_i(_pid2_rate_roll.kI() * angle_error_rads * _conv / 1000.0f + _pid2_rate_roll_tilt.kI() * angle_error_rads * (1000 - _conv) / 1000.0f, _dt);
    float d = _pid2_rate_roll.kP() * rate_error_rads * _conv / 1000.0f + _pid2_rate_roll_tilt.kP() * rate_error_rads * (1000 - _conv) / 1000.0f;

    return constrain_float((p + i + d) / 4500.f, -1.0f, 1.0f);
}

float AC_AttitudeControl_TiltQuad::aeroxo_rate_bf_to_motor_pitch(float rate_target_rads)
{
    float current_rate_rads = _ahrs.get_gyro().y;
    float rate_error_rads = rate_target_rads - current_rate_rads;
    float angle_error_rads = _att_error_rot_vec_rad.y;

    float p = _pi_stabilize_pitch.kP() * angle_error_rads * _conv / 1000.0f + _pi_stabilize_pitch_tilt.kP() * angle_error_rads * (1000 - _conv) / 1000.0f;
    float i = _pi_stabilize_pitch.get_i(_pid2_rate_pitch.kI() * angle_error_rads * _conv / 1000.0f + _pid2_rate_pitch_tilt.kI() * angle_error_rads * (1000 - _conv) / 1000.0f, _dt);
    float d = _pid2_rate_pitch.kP() * rate_error_rads * _conv / 1000.0f + _pid2_rate_pitch_tilt.kP() * rate_error_rads * (1000 - _conv) / 1000.0f;

    return constrain_float((p + i + d) / 4500.f, -1.0f, 1.0f);
}

float AC_AttitudeControl_TiltQuad::aeroxo_rate_bf_to_motor_yaw(float rate_target_rads)
{
    float current_rate_rads = _ahrs.get_gyro().z;
    float rate_error_rads = rate_target_rads - current_rate_rads;
  
    float p = _pi_stabilize_yaw.kP() * rate_error_rads *_conv / 1000.0f + _pi_stabilize_yaw_tilt.kP() * rate_error_rads * (1000 - _conv) / 1000.0f;
    float i = _pi_stabilize_yaw.get_i(_pid2_rate_yaw.kI() * rate_error_rads * _conv / 1000.0f + _pid2_rate_yaw_tilt.kI() * rate_error_rads * (1000 - _conv) / 1000.0f, _dt);
	
    return constrain_float((p + i) / 4500.f, -1.0f, 1.0f);
}

// relax_bf_rate_controller - ensure body-frame rate controller has zero errors to relax rate controller output
void AC_AttitudeControl_TiltQuad::relax_bf_rate_controller()
{
    AC_AttitudeControl::relax_bf_rate_controller();

    _pi_stabilize_roll.reset_I();
    _pi_stabilize_pitch.reset_I();
    _pi_stabilize_yaw.reset_I();

    _pi_stabilize_roll_tilt.reset_I();
    _pi_stabilize_pitch_tilt.reset_I();
    _pi_stabilize_yaw_tilt.reset_I();

    _pid2_rate_roll.reset_I();
    _pid2_rate_pitch.reset_I();
    _pid2_rate_yaw.reset_I();

    _pid2_rate_roll_tilt.reset_I();
    _pid2_rate_pitch_tilt.reset_I();
    _pid2_rate_yaw_tilt.reset_I();
}

void AC_AttitudeControl_TiltQuad::loadAeroxoTiltrotorParameters()
{
    ElytraConfigurator elCfg;
    printf("Scanning XML Elytra config...\n");
    elCfg.scanSetupFile();

    if (elCfg.getOkLoad()) {
        _pi_stabilize_roll=APM_PI2(elCfg.getPRoll(),0,elCfg.getIMaxRoll());
        _pi_stabilize_pitch=APM_PI2(elCfg.getPPitch(),0,elCfg.getIMaxPitch());
        _pi_stabilize_yaw=APM_PI2(elCfg.getPYaw(),0,elCfg.getIMaxYaw());

        _pi_stabilize_roll_tilt=APM_PI2(elCfg.getPRollTilt(),0,elCfg.getIMaxRoll());
        _pi_stabilize_pitch_tilt=APM_PI2(elCfg.getPPitchTilt(),elCfg.getIMaxPitch());
        _pi_stabilize_yaw_tilt=APM_PI2(elCfg.getPYawTilt(),0,elCfg.getIMaxYaw());

        _pid2_rate_roll=AC_PID2(elCfg.getDRoll(),elCfg.getIRoll(),0);
        _pid2_rate_pitch=AC_PID2(elCfg.getDPitch(),elCfg.getIPitch(),0);
        _pid2_rate_yaw=AC_PID2(0,elCfg.getIYaw(),0);

        _pid2_rate_roll_tilt=AC_PID2(elCfg.getDRollTilt(),elCfg.getIRollTilt(),0);
        _pid2_rate_pitch_tilt=AC_PID2(elCfg.getDPitchTilt(),elCfg.getIPitchTilt(),0);
        _pid2_rate_yaw_tilt=AC_PID2(0,elCfg.getIYawTilt(),0);

        printf("Elytra: loaded parameters from XML!\n");
    }
    else {
        _pi_stabilize_roll       = APM_PI2(1700,    0, 600);
        _pi_stabilize_pitch      = APM_PI2(1700,    0, 600);
        _pi_stabilize_yaw        = APM_PI2( 500,    0, 600);

        _pi_stabilize_roll_tilt  = APM_PI2(1200,    0, 600);
        _pi_stabilize_pitch_tilt = APM_PI2(1200,    0, 600);
        _pi_stabilize_yaw_tilt   = APM_PI2(1000,    0, 600);

        _pid2_rate_roll          = AC_PID2( 600, 3300,   0);
        _pid2_rate_pitch         = AC_PID2( 600, 3300,   0);
        _pid2_rate_yaw           = AC_PID2(   0, 1000,   0);

        _pid2_rate_roll_tilt     = AC_PID2( 400, 2300,   0);
        _pid2_rate_pitch_tilt    = AC_PID2( 400, 2300,   0);
        _pid2_rate_yaw_tilt      = AC_PID2(   0, 2000,   0);

        printf("Elytra: loaded default parameters!\n");
    }
}
