...
/// @file	AC_PID2.h
/// @brief	Generic PID2 algorithm, with EEPROM-backed storage of constants.

#ifndef __AC_PID2_H__
#define __AC_PID2_H__
...
public:
    ...
    /// Iterate the PID2, return the new control value
    ///
	...
	int32_t         get_it2(int32_t error, float dt);
    int32_t         get_d(int32_t error, float dt);
	int32_t 		get_leaky_i(int32_t error, float dt, float leak_rate);
	int32_t 		get_leaky_it2(int32_t error, float dt, float leak_rate);

    /// Reset the PID2 integrator
    ///
    void        reset_I();
