...
#include <aeroxo/equipment/genset/ecu/Status2.hpp>
...
public:
    ...
    static AP_Generator *get_singleton() { return _singleton; }
...
private:
    ...
    bool _batt_healthy, _starter_healthy, _ecu_healthy;

    bool is_healthy() const { return _batt_healthy && _starter_healthy && _ecu_healthy; }
    void report_unhealthy();

    float _gen_current;
    int32_t _rpm, _rpm_alt;

    int _fuel_lvl_pct;
    float _fuel_lvl_raw;

    float _ice_temp = NAN, _gen_temp = NAN, _vsi_temp = NAN, _ice_temp_alt = NAN;
    ...
    void update_should_run(bool should_run);
    ...
    void ecuStatus2Callback(const aeroxo::equipment::genset::ecu::Status2 &msg);
