        ...
        if (state[i].uart != NULL) {
            switch (state[i].protocol) {
                ...
                case SerialProtocol_SToRM32:
                    ...
                    break;
                case SerialProtocol_SES:
                    // Note baudrate is hardcoded to 57600
                    state[i].baud = AP_SERIALMANAGER_SES_BAUD / 1000;   // update baud param in case user looks at it
                    state[i].uart->begin(map_baudrate(state[i].baud),
                                         AP_SERIALMANAGER_SES_BUFSIZE_RX,
                                         AP_SERIALMANAGER_SES_BUFSIZE_TX);
                    break;
            }
        }
    }
